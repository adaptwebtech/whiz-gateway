...
> Feature 13. Recebe webhooks de Instagram/Messenger da Meta em rotas dedicadas (`GET/POST /webhook/{instagram,instagram-login,messenger,messenger-login}`) e faz passthrough cru (byte-idêntico, preservando `x-hub-signature-256`) para o servidor whiz-v2 do ambiente correto, com retry exponencial e DLQ em falha. Superfície fixada pela rota (não pelo `object`); tenant por `entry[0].id` (IGID / pageId). Verify token por app: `META_VERIFY_TOKEN` (instagram/messenger), `IG_VERIFY_TOKEN` (instagram-login), `MESSENGER_LOGIN_VERIFY_TOKEN` (messenger-login, app Meta dedicado). Sem verificação de HMAC no gateway (passthrough-only). Sem alteração de schema (`pid` = IGID/pageId). Spec: [`docs/specs/2026-07-01-instagram-webhook-redirect.md`](../specs/2026-07-01-instagram-webhook-redirect.md) · Impl: [`docs/implementation/2026-07-01-instagram-webhook-redirect.md`](../implementation/2026-07-01-instagram-webhook-redirect.md) · ADR: [`docs/adr/0001-instagram-webhook-passthrough.md`](../adr/0001-instagram-webhook-passthrough.md) · Status: Implementada (+ `messenger-login` 2026-07-27)
