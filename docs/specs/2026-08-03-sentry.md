...
`http://31.97.27.185:30808`, projeto `2` (imagem `glitchtip/glitchtip:6.2.2`).
Ela ingere **eventos de erro**, **transações de performance** e **logs
estruturados** (ingestão de logs ligada por padrão no servidor,
`GLITCHTIP_ENABLE_LOGS`). **Não** implementa _release health/sessions_ e não
documenta ingestão da API de _trace metrics_ — por isso as métricas de domínio
viajam como atributos de uma transação (§9).
...
- Desligamento explícito de sessões (o GlitchTip não as suporta).
- Logs estruturados (`enableLogs`) e trace metrics (`enableMetrics`) ligados por
  env; logs alimentados pela ponte oficial winston→Sentry.
...
- **Snapshot de métricas**: transação sintética emitida a cada 60s com os
  ...
  `Sentry.metrics.*`, cuja ingestão o GlitchTip não documenta).
- **Log estruturado**: entrada da página *Logs* do GlitchTip — nível, mensagem e
  atributos, correlacionada ao rastro pelo `trace_id`. _Avoid_: breadcrumb (só
  existe anexada a um evento) e issue (log `error` gera as duas coisas).
...
- **FR-7** — `enableLogs` e `enableMetrics` seguem `SENTRY_ENABLE_LOGS` /
  `SENTRY_ENABLE_METRICS` (padrão `false`; em produção ambos ligados).
...
- **FR-19** — `LoggerService` inclui o transport oficial de logs
  (`createSentryWinstonTransport`) restrito aos níveis `info`, `warn`, `error` e
  `fatal` — `debug`/`verbose`/`silly` não sobem. O transport só produz envelope
  quando `enableLogs` está ligado (garantia do próprio SDK).
- **FR-20** — `LoggerService` só anexa o metadado `optionalParams` quando há
  parâmetros, para não poluir os atributos do log com array vazio.
...
- `Sentry.metrics.*` chamado com `enableMetrics=false` → no-op do SDK
  (chamadas mantidas para compatibilidade futura, sem custo). O mesmo vale para
  o transport de logs com `enableLogs=false`.
- Log de nível `error` com logs ligados gera **duas** entradas no GlitchTip: a
  issue (agrupa e alerta) e a linha de log (contexto navegável). É intencional.
...
- **AC-19** `[backend]`: Given o `LoggerService` instanciado, when os transports
  do Winston são inspecionados, then existem console, ponte de eventos
  (`SentryWinstonTransport`) e transport de logs criado por
  `createSentryWinstonTransport` com `levels: ['info','warn','error','fatal']`.
- **AC-20** `[backend]`: Given um log sem parâmetros extras, when
  `LoggerService.error(msg)` é chamado, then o Winston recebe apenas a mensagem;
  given `LoggerService.error(msg, marcador)`, then recebe
  `{ optionalParams: [marcador] }`.
